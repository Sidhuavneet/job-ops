...
// ── Freshness filter ────────────────────────────────────────────────
// Optional. Drops postings older than `freshness.max_age_days` (portals.yml)
// using the provider's `postedAt` (ISO string from Greenhouse/Ashby/Lever).
// Returns true (pass) when:
//   - no freshness config / non-positive max_age_days → filter disabled
//   - postedAt is missing or unparseable → don't penalize missing provider data
export function buildFreshnessFilter(freshness) {
  const maxAgeDays = Number(freshness?.max_age_days);
